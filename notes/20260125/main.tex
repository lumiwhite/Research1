...
\section{論文\cite{kasai2026breakingorthogonalitybarrierquantum}について}
...
とあり、サイクルの隣り合う要素は行か列いずれか1つが異ならなければならない。よって、行インデックスシーケンス、列インデックスシーケンスともに同じ要素が連続してはいけない、という前提とする。
...
\section{2/4 ゼミ}
\cite{kasai2026breakingorthogonalitybarrierquantum}では、以下の関数がガースが8となるような$F_i, G_i$として挙げられた。
\begin{figure}[htbp]
    \includegraphics{image.png}
\end{figure}
しかし、Pythonプログラムでこの$\aU$から$\bU$を求めようとしても解が見つからないので、テストしたら、閉ブロックサイクルが見つかった。

サイクル:$[0, 0], [0, 7], [3, 7], [3, 4], [2, 4], [2, 0]$について、サイクルの関数は$f\inv(x) = f_0\inv g_1 g_4\inv f_1 f_2\inv f_4  = 209x+ 640$であり、$x=640$を解にもつ。
\subsection{確認したいこと}
\begin{itemize}
  ...
  \item 現状、長さ6以下のサイクルを含まない、という条件なら20秒ほどで探索が終わるが、解なし。長さ8のUTCBC以外のサイクルを含まない、という条件での探索だと、7分ほどかかる。
...
\begin{itemize}
  \item 整数計画法で特殊解を求める。
  \item スミス標準形を勉強
  \item 100万ほどの制約条件をすべて考慮しないといけないので、効率的な方法を考える。
\end{itemize}
\section{スミス標準形 (Smith Normal Form, SNF) による一般解の導出}

本研究で扱うアフィン置換行列（APM）の可換性条件、あるいは直交性条件は、法 $P$ における線形合同式体系として記述される。ここでは、スミス標準形を用いてその一般解を導出する手法を整理する。

\subsection{線形合同式体系の定式化}
