...
\begin{itemize}
  ...
  \end{align}
\end{itemize}
$\aU=[\aU_f, \aU_g]\tr=[a_{f,0},a_{f,1},a_{f,2},a_{g,0},a_{g,1},a_{g,2}]\tr,\bU=[\bU_f, \bU_g]\tr=[b_{f,0},b_{f,1},b_{f,2},b_{g,0},b_{g,1},b_{g,2}]\tr$とする。
...
まずは簡単のため、$a_{f,i}\neq 0$、$a_{g,i}\neq 0$として考える。
式\ref{eq:cond_a_mat}を解くと、基底$\vU_0,\dots,\vU_5$及係数$\cU=[c_0,\dots,c_5]\tr$により、解空間として
\begin{align}
  \bU = \sum_{j=0}^5 c_j \vU_j = V\cU
\end{align}
が求められる。ただし、$V=[\vU_0,\dots,\vU_5^\top],c_i\in[P]$である。
これにより、式\ref{eq:cond_b_mat_f1}と\ref{eq:cond_b_mat_g1}は
\begin{align}
  \begin{bmatrix}
        1-a_{f,1} & a_{f,0}-1 & 0 & 0 & 0 & 0\\ %i=0,j=1
        1-a_{f,2} & 0 & a_{f,0}-1 & 0 & 0 & 0\\ %i=0,j=2
...
\end{align}
これを$\equiv \zeroU$として解くことで、条件bを満たさない係数$\cU$の空間が決定し、これを避けることで条件a,bを満たす$\aU,\bU$が求まる。
...
\begin{align}
  \cy{X}{[0,1]}{0} = 
    ...
    f_0
  \end{align}
  となり、サイクルの関数$f_{\cy{X}{[0,1]}{0}}(x)$について
\begin{align}
    f_{\cy{X}{[0,1]}{0}}\inv(x) = 
    f_0\inv
    f_1
...
\end{align}
が成り立つ。
ここで、$f_i(x) = a_{f,i}x + b_{f,i}$および$f_i\inv(x) = a_{f,i}\inv(x - b_{f,i})$であるから、
 \begin{align}
    f_{\cy{X}{[0,1]}{0}}\inv(x) &= f_0\inv f_1f_0\inv f_2(x)\\
    &= a_{f,0}\inv(a_{f,1}a_{f,0}\inv(a_{f,2}x + b_{f,2} - b_{f,0}) + b_{f,1} - b_{f,0})\\
    &= a_{f,0}\inv a_{f,1}a_{f,0}\inv a_{f,2} x + a_{f,0}\inv a_{f,1}a_{f,0}\inv (b_{f,2} - b_{f,0}) + a_{f,0}\inv (b_{f,1} - b_{f,0})\\
    &= a_{f,0}\inv a_{f,1}a_{f,0}\inv a_{f,2} x -  
    a_{f,0}\inv (1+a_{f,1}a_{f,0}\inv)b_{f,0} + a_{f,0}\inv (b_{f,1}) + a_{f,0}\inv a_{f,1}a_{f,0}\inv (b_{f,2})
\end{align}
\begin{definition}
  ここで、サイクル$\cy{X}{\ell}{0}$の関数$f_{\cy{X}{\ell}{0}}(x)$について、
  \begin{align}
    f_{\cy{X}{\ell}{0}}(x) = A_{\cy{X}{\ell}{0}}x+B_{\cy{X}{\ell}{0}}
  \end{align}
  と表すこととする。
\end{definition}
つまり、
\begin{align}
  A_{\cy{X}{[0,1]}{0}} &= a_{f,0}\inv a_{f,1}a_{f,0}\inv a_{f,2}\\
  B_{\cy{X}{[0,1]}{0}} &= - a_{f,0}\inv (1+a_{f,1}a_{f,0}\inv)b_{f,0} + a_{f,0}\inv b_{f,1} + a_{f,0}\inv a_{f,1}a_{f,0}\inv b_{f,2}
\end{align}
である。
このブロックサイクルが閉でないための条件は、
\begin{align}
  B_{\cy{X}{[0,1]}{0}} \not\equiv 0 \pmod{\gcd(A_{\cy{X}{[0,1]}{0}}-1, P)}
\end{align}
