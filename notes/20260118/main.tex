...
我々は、$H_XH_Z\top=0$の十分条件の便利な形を再び導く。
\begin{align}
...
\begin{table}[htbp]
  \centering
  \caption{構成された APM パラメータ ($P = 768, J = 3, L = 12, \text{girth}=8$)}
  \begin{tabular}{c|l|l}
    \hline
    $i$ & $f_i(x)$ & $g_i(x)$ \\ \hline
    0 & $763x + 435$ & $289x + 496$ \\
    ...
    3 & $61x + 18$ & $41x + 524$ \\
    4 & $697x + 612$ & $193x + 672$ \\
    5 & $373x + 246$ & $449x + 672$ \\ \hline
  \end{tabular}
\end{table}

$J = 3, L = 12$ のタナーグラフにおいて、ブロック位置
\[
[(0, 0), (0, 6), (1, 6), (1, 1), (2, 1), (2, 7), (1, 7), (1, 0)]
\]
は、順番に辿ると 8-サイクルを形成する。式\eqref{eq:01201453}の形式による、対応するサイクルワードは以下の通りである。
\[
...
$L/2 = 6$ および $J = 3$ のとき、$\Delta = \{0, 1, 2, 4, 5\}$ である（したがって $3 \notin \Delta$）。設計目標は以下の通りである。
\[
\Psi_r = 0 \quad (r \in \Delta), \quad \Psi_3 \neq 0
\]
これは、マザー直交性（mother orthogonality）を破りつつアクティブ直交性（active orthogonality）を保証し、それによって潜在的な部分（latent part）がアクティブな部分（active part）と直交するのを防ぐ余地を作るものである。

略図的な交換表（commutation table）として、$(i, j) = (0, 3), (1, 2)$ を除いて、すべてのペア $(F_i, G_j)$ が交換（commute）すると仮定する。以下の表では、$\Gamma$ 内の交換するペアを 1 で網掛けしている：
...
\]
読者は、$\Psi_4 = 0$ と設定しながら $\Psi_3$ のみを非ゼロに保てば十分であると考えるかもしれない。我々は当初これを検討し、$F_0, G_3$ のみが非交換（non-commuting）となるように符号を設計した。その結果、$\Psi_3 \neq 0$ かつ $\Psi_4 = 0$ となった。しかし、これでは $k$ 個の長さ 8 のサイクルを接続することによって形成される多くの基本的トラッピングセット（ETS）が生成され、これらは最も有害である [6]。代表的な ETS を図 1 に示す。代わりに $\Psi_3 \neq 0$ かつ $\Psi_4 \neq 0$ となるように符号を設計し、$F_0, G_3$ と $F_1, G_2$ の両方を非交換にしたところ、$k \le 10$ における連結された ETS を大幅に削減できた。

\section*{6.3 アフィン置換の構成と $P$ の選択}

アルゴリズムによって構成されたアフィン置換 $f_i, g_i$ は表 1 に記載されている。$P$ が素数冪である場合、以下の条件を同時に満たす $[P]$ 上の置換の集合 $\sigma_0, \sigma_1, \tau_0, \tau_1$ は存在しない：$\sigma_0$ は $\tau_0$ と交換するが $\tau_1$ とは交換せず、$\sigma_1$ は $\tau_0$ とは交換しないが $\tau_1$ とは交換する。$AGL_1(\mathbb{Z}/p^k \mathbb{Z})$ の標準的な半直積表現 $(a, b) : x \mapsto ax + b$ および二次交換条件を用いることで、結果として得られる $p$-進付値を持つゼロ積条件を分析することにより、これを示すことができる。これは非交換性と矛盾する。したがって、我々は $P = 768 = 3 \times 2^8$ を選択する。

\begin{figure}[h]
\centering
% (a) と (b) の図のプレースホルダー
\caption{長さ 8 のサイクルを $k = 2, 5$ 個接続することで形成された ETS。塗りつぶされたチェックノードは次数 1（未充足）である。$P = 768, J = 3, L = 12$ におけるカウント：(6, 2) ETS ($X=48, Z=16$) および (12, 2) ETS ($X=23, Z=0$)。}
\end{figure}

\begin{figure}[h]
\centering
% 図 2 のプレースホルダー
\caption{(8, 2) を持つ ETS のタナーグラフ。内部サブグラフは図 1(a) の (6, 2) ETS と一致し、長さ 4 のパスがその以前の次数 1 のチェック（C0:599 と C0:542）を接続している。$P = 768, J = 3, L = 12$ におけるカウント：(8, 2) ETS ($X=48, Z=0$)。}
\end{figure}

\section*{6.4 ETS ライブラリの構築}

初期の実験において、図 1 に示される形式の ETS は信念伝搬（BP）法における失速の主要な原因であった。構成された符号はガース 8 を持つため、まず長さ 8 のサイクルを列挙し、次にこれらのパターンと同型な ETS を生成するために 8-サイクルを接続することによって ETS ライブラリを構築する。ライブラリは単にデコード中に遭遇した ETS を集めたものではなく、支配的なパターンと同型なすべての ETS を列挙し、それらを網羅的に追加したものである。

図 1(a) は 2 つの長さ 8 のサイクルを接続して得られた (6, 2) ETS であり、図 1(b) は 5 つの長さ 8 のサイクルを接続して得られた (12, 2) ETS である。図 2 は、(6, 2) ETS の 2 つの奇数チェック間に長さ 4 のチェック-変数パスを付加することによって形成された (8, 2) ETS を示している。$P = 768, J = 3, L = 12$ の構成符号について、列挙アルゴリズムは以下を見出した：(6, 2) ETS: $X = 48, Z = 16$ (合計 64); (12, 2) ETS: $X = 23, Z = 0$ (合計 23); そして (8, 2) ETS (path4 タイプ): $X = 48, Z = 0$ (合計 48)。
