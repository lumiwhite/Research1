...
\section{今後の方針}
\begin{itemize}
  \item 複数の特殊解について、長さ8以上のサイクルの個数を比較し、性能の違いを評価する。
  \item $\aU$をランダムに生成し、それに対して整数計画法で特殊解を求め、性能を示すものを探す。また、UTCBC以外の長さ8のサイクルを含まないものが見つけられたらうれしい。
...
\section{研究方針：ガース6のプロトグラフへの転換}
本研究では、全結合プロトグラフの前提を崩し、ベース行列 $B$ そのものにスパース構造を導入することでガース10を論理的に可能にする。

\subsection{1. ベース行列 $B$ の RC-condition 充足}
...
\subsection{2. 直交条件（Condition A）の再定式化}
...
\subsection{3. 長さ 6, 8, 10 サイクル条件の Z3 による解決}
...
\section{行選択（Masking）の導入：固定から動的へ}
従来の構成では、プロトグラフの全要素にAPMを配置する全結合型を採用していた。これに対し、大きなベース行列 $\bm{B}_{base} \in \{0, 1\}^{M \times N}$ を定義し、そこから特定の $1$ を「選択（Masking）」する手法を導入することで、グラフのトポロジーを制御する。

選択行列（Masking Matrix） $\bm{Z} = \{z_{i,j}\}$ を以下のように定義する。
\begin{equation}
    H = \sum_{i=1}^{M} \sum_{j=1}^{N} z_{i,j} \bm{P}_{a_{i,j}, b_{i,j}}, \quad z_{i,j} \in \{0, 1\}
\end{equation}
ここで、$z_{i,j}=1$ の場合のみ APM が配置され、$z_{i,j}=0$ は零行列（接続なし）を意味する。

\section{RC条件（Row-Column Condition）による4-サイクルの根絶}
プロトグラフ段階でのガースを 6 以上にするために、選択行列 $\bm{Z}$ に対して以下の **RC条件** を課す。
\begin{equation}
    \forall i, i' \in \{1, \dots, M\}, \quad \bm{z}_i \cdot \bm{z}_{i'} = \sum_{j=1}^{N} z_{i,j} z_{i',j} \le 1
\end{equation}
この条件により、ベース行列内で 2 つの行が 2 箇所以上の列で同時に接続を持つ（＝長方形を形成する）ことが物理的に不可能となる。

\section{直交条件の緩和と探索空間の拡大}
行の取り出し方を列ごとに分散（スパース化）させることで、量子直交条件 $H_x H_z^T = 0$ に寄与するペアが限定される。

\subsection{制約行列 \texorpdfstring{$\bm{G}_{mask}$}{G\_mask} の疎性}
全結合時と比較し、直交条件行列 $\bm{G}_{mask}$ の行数は大幅に削減される。
\begin{equation}
    \text{rank}(\bm{G}_{mask}) < \text{rank}(\bm{G}_{full})
\end{equation}
この結果、解空間（null space）の次元 $d_a$ が増大し、条件C（ガース10）を同時に満たす特殊解 $\bm{b}$ が存在する「隙間」が拡大する。

\section{ガース10達成への論理的ステップ}
\begin{enumerate}
    \item \textbf{プロトグラフ・ガースの向上：} $\bm{Z}$ の設計により、リフティング後の「構造的な8-サイクル」の発生源（ベースの4-サイクル）を完全に削除する。
    \item \textbf{動的行選択の最適化：} 各列の重み $J=3$ を維持しつつ、RC条件を満たす最適な行インデックスの組み合わせを PEG アルゴリズム等で算出する。
    \item \textbf{一般解の再探索：} 拡張された自由度 $d_a$ を活用し、長さ 6, 8, 10 のパスベクトル $\bm{r}_k$ をすべて回避する $\bm{b}$ を Z3 で特定する。
\end{enumerate}
