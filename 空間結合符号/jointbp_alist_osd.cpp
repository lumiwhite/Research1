...
static void update_check_llr(
    ...
    for (size_t c = 0; c < v2c.size(); ++c) {
        ...
        for (size_t i = 0; i < deg; ++i) t[i] = std::tanh(0.5 * v2c[c][i]);
...
static GF2SolveInfo solve_gf2_bits(
    ...
    for (int c = 0; c < nvars && r < m; ++c) {
        ...
        for (int i = 0; i < m; ++i) {
            ...
            for (int k = 0; k < words; ++k) rows[i][k] ^= rows[r][k];
            rhs[i] ^= rhs[r];
        }
        ++r;
    }
    ...
    for (int i = r; i < m; ++i) {
        ...
        for (int k = 0; k < words; ++k) {
            if (rows[i][k] != 0) { zero = false; break; }
        }
        if (zero && (rhs[i] & 1)) {
            info.solvable = false; info.rank = r;
            return info;
        }
    }
    
    info.solvable = true; info.rank = r;
...
static bool attempt_osd(
    ...
    int nvars = abs_llr.size();
    std::vector<int> order(nvars);
    for(int i=0; i<nvars; ++i) order[i] = i;
    
    // LLRの絶対値が小さい順（信頼度が低い順）にソート
    std::sort(order.begin(), order.end(), [&](int a, int b) {
        return abs_llr[a] < abs_llr[b];
    });

    auto current_synd = compute_syndrome(checks, est, use_xbit);
    std::vector<int> diff(checks.size(), 0);
    bool any_diff = false;
    for(size_t i=0; i<checks.size(); ++i) {
        diff[i] = current_synd[i] ^ target_syndrome[i];
        if(diff[i]) any_diff = true;
    }
    if(!any_diff) return true;

    // k個の「最も疑わしい」ビットを集めて部分行列を作り、ガウス消去法でエラーを特定する
    for (int k = std::min(20, max_k); k <= max_k; k += std::max(20, k/2)) {
        if (k > nvars) break;
        
        std::vector<int> pos(nvars, -1);
        ...
        for(size_t c=0; c<checks.size(); ++c) {
            ...
            for(int v : checks[c]) {
                int p = pos[v];
                if (p >= 0) row[p >> 6] |= (1ULL << (p & 63));
            }
            bool empty = true;
            for(int w=0; w<words; ++w) if(row[w] != 0) { empty = false; break; }
            
            if (empty) {
                if (diff[c] != 0) {
                    possible = false;
                    break; // k個の変数だけではこのシンドロームを解消できない
                }
            } else {
        ...
        auto info = solve_gf2_bits(std::move(rows), std::move(rhs), k, delta);
        if (info.solvable) {
            std::vector<int> trial = est;
            int flips = 0;
            for(int i=0; i<k; ++i) {
                if (delta[i]) {
                    trial[order[i]] ^= (use_xbit ? 1 : 2);
                    flips++;
                }
            }
            ...
            if (new_synd == target_syndrome) {
                est = trial;
                if (verbose) {
                    std::cout << "  [OSD " << side << "] K=" << k << " Solved! (Flipped " << flips << " bits)\n";
                }
                return true;
            }
        }
...
static JointBPResult joint_bp_decode(
    ...
    std::vector<double> abs_llr_x(nvars, 0.0);
    std::vector<double> abs_llr_z(nvars, 0.0);
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        for (int v = 0; v < nvars; ++v) {
            ...
            // BPが限界に達する直前のイテレーションで、各ビットのLLRの絶対値（信頼度）を保存する
            if (iter == max_iter - 1) {
                ...
                abs_llr_x[v] = std::abs(log_x0_m - log_x1_m);
                abs_llr_z[v] = std::abs(log_z0_m - log_z1_m);
            }

            for (const auto &e : var_to_x[v]) {
                x_v2c[e.check][e.pos] = (logsumexp2(log_phi00 + log_z0, log_phi01 + log_z1) -
                                         logsumexp2(log_phi10 + log_z0, log_phi11 + log_z1)) + 
                                         (Lx_sum - x_c2v[e.check][e.pos]);
            }
            for (const auto &e : var_to_z[v]) {
                z_v2c[e.check][e.pos] = (logsumexp2(log_phi00 + log_x0, log_phi10 + log_x1) -
        ...
        int us_x = 0, us_z = 0;
        for (size_t c = 0; c < x_checks.size(); ++c) if (sx_hat[c] != sx[c]) us_x++;
        for (size_t c = 0; c < z_checks.size(); ++c) if (sz_hat[c] != sz[c]) us_z++;
        ...
        if (us_x == 0 && us_z == 0) {
            return {est, iter + 1, true, false}; // BPのみで解決
        }
    }

    // BPが収束しなかった場合、OSD（順序統計復号）を発動
    if (osd_order > 0) {
        if (verbose) std::cout << "  -> BP Stuck. Triggering OSD...\n";
        bool osd_x = attempt_osd("X", abs_llr_x, x_checks, sx, est, true, osd_order, verbose);
        bool osd_z = attempt_osd("Z", abs_llr_z, z_checks, sz, est, false, osd_order, verbose);
        
        auto sx_hat = compute_syndrome(x_checks, est, true);
        auto sz_hat = compute_syndrome(z_checks, est, false);
        if (sx_hat == sx && sz_hat == sz) {
            return {est, max_iter, true, true}; // OSDによって解決
        }
    }

    return {est, max_iter, false, false};
}
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " --hx <H_X.alist> --hz <H_Z.alist> --p <error_rate> --trials <trials> [--max-iter <max_iter>] [--osd-order <k>] [--verbose]\n";
        return 1;
    }
    ...
    int max_iter = 50;
    int trials = 1000;
    int osd_order = 300; // デフォルトで300ビットまで探索
    bool verbose = false;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--osd-order" && i + 1 < argc) osd_order = std::stoi(argv[++i]);
    ...
    std::cout << "p = " << p_err << ", trials = " << trials << ", max_iter = " << max_iter << ", osd_order = " << osd_order << "\n\n";
    ...
    for (int t = 0; t < trials; ++t) {
        ...
        if (verbose) std::cout << "=== Trial " << t + 1 << " / " << trials << " ===\n";

        auto sx = compute_syndrome(x_checks, err, true);
        auto sz = compute_syndrome(z_checks, err, false);

        auto res = joint_bp_decode(x_checks, z_checks, var_to_x, var_to_z, sx, sz, prior, max_iter, osd_order, verbose);

        total_iters += res.iterations;

        if (!res.syndrome_match) {
            failures++;
        } else if (res.used_osd) {
            osd_successes++;
        }
    ...
    std::cout << "OSD Success: " << osd_successes << " (BPが失敗し、OSDが救済した回数)\n";
    std::cout << "FER        : " << fer << "\n";
