...
struct Params {
    ...
    std::vector<int> active_set; // ★この1行を追加
};
...
static BlockMatrix build_block_matrix_hx(
    ...
    const std::vector<int> &active_set // ★追加
) {
    int L = 2 * L2;
    BlockMatrix blocks(J, std::vector<std::optional<APM>>(L));
    for (int r = 0; r < J; ++r) {
        int active_r = active_set[r]; // ★追加: 実際の行番号を取得
        for (int cc = 0; cc < L; ++cc) {
            if (cc < L2) {
                int idx = mod_index(cc - active_r, L2); // ★ r を active_r に変更
                blocks[r][cc] = APM{mod_norm(a[idx], P), mod_norm(b[idx], P)};
            } else {
                int t = cc - L2;
                int idx = mod_index(t - active_r, L2); // ★ r を active_r に変更
...
static BlockMatrix build_block_matrix_hz(
    ...
    for (int r = 0; r < J; ++r) {
        ...
        for (int cc = 0; cc < L; ++cc) {
            if (cc < L2) {
                int k = mod_index(cc - active_r, L2); // ★ r を active_r に変更
                ...
                int t = cc - L2;
                int k = mod_index(t - active_r, L2); // ★ r を active_r に変更
                int idx = mod_index(-k, L2);
...
static bool read_params(const std::string &path, Params &out) {
    ...
    std::string word;
    while (in >> word) {
        if (word.find("active_set=") == 0) {
            std::string vals = word.substr(11);
            std::stringstream ss(vals);
            std::string item;
            int idx = 0;
            while (std::getline(ss, item, ',') && idx < out.J) {
                out.active_set[idx++] = std::stoi(item);
            }
        }
    }
    return true;
...
int main(int argc, char **argv) {
    ...
    std::vector<int> cmd_active_set;
    bool cmd_active_set_provided = false;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--params") {
            ...
            simulate = true;
            if (arg == "--active" && i + 1 < argc) {
                std::string vals = argv[++i];
                std::stringstream ss(vals);
                std::string item;
                while (std::getline(ss, item, ',')) {
                    cmd_active_set.push_back(std::stoi(item));
                }
                cmd_active_set_provided = true;
            }
        // --- ここまで差し替え ---
    ...
    if (cmd_active_set_provided) {
        params.active_set = cmd_active_set;
    }
    ...
    auto hx_blocks = build_block_matrix_hx(params.P, params.J, params.L2, params.a, params.b, params.c, params.d, params.active_set);
    auto hz_blocks = build_block_matrix_hz(params.P, params.J, params.L2, params.a, params.b, params.c, params.d, params.active_set);
...
/*
./jointbp_ets \
  --params P_576_20260408_201624.txt \
