...
#include <io.h>
#else
#include <unistd.h>  // ★この行が isatty エラーを直します
#endif
...
public:
    TeeBuf(std::streambuf* orig_buf) : orig_buf(orig_buf) {
        file_lines.push_back(""); // 初期行
    }

    // 蓄積した最終結果のログをファイルに一括書き出しする関数
    void write_to_file(std::ofstream& ofs) {
        if (!current_line.empty()) {
            file_lines.back() += current_line;
        }
...
protected:
    virtual int overflow(int c) {
        ...
        // 1. 画面には常にそのままリアルタイム出力
        int r1 = orig_buf->sputc(c);

        // 2. ファイル用内部バッファの処理（カーソル上移動の再現）
        if (in_csi) {
            csi_seq += (char)c;
            if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
                in_csi = false;
                // 'A' はカーソルを上へ移動する命令 (例: \x1B[14A)
                if (c == 'A') {
                    int n = 1;
                    if (csi_seq.size() > 1) {
                        try { n = std::stoi(csi_seq.substr(0, csi_seq.size() - 1)); } catch(...) {}
                    }
                    // 指定された行数分だけ、記録したログから行を削除（上書き動作）
                    for (int i = 0; i < n; ++i) {
                        if (file_lines.size() > 1) file_lines.pop_back();
                    }
                    current_line = ""; 
                }
            }
...
static BlockMatrix build_block_matrix_hx(
    ...
    for (int r = 0; r < J; ++r) {
        int row = r;
        if (!row_set.empty()) {
            row = row_set[static_cast<size_t>(r)];
        }
        for (int cc = 0; cc < L; ++cc) {
            if (cc < L2) {
                int idx = mod_index(cc - row, L2);
                blocks[r][cc] = APM{mod_norm(a[idx], P), mod_norm(b[idx], P)};
            } else {
                int t = cc - L2;
                int idx = mod_index(t - row, L2);
...
static bool parse_row_set(const std::string &text, int J, int L2,
...
static bool read_params(const std::string &path, Params &out) {
    ...
    while (std::getline(in, line)) {
        size_t pos = line.find("active_set=");
        const char *label = "active_set";
        if (pos == std::string::npos) {
            pos = line.find("active_row=");
            label = "active_row";
        }
        if (pos == std::string::npos) continue;
        std::string value = line.substr(pos + std::strlen(label) + 1);
        size_t hash = value.find('#');
        if (hash != std::string::npos) value = value.substr(0, hash);
        std::string err;
        std::vector<int> parsed;
        if (!parse_row_set(value, out.J, out.L2, parsed, err)) {
            std::cerr << "Invalid " << label << " in " << path << ": " << err << "\n";
            return false;
        }
        out.active_set = std::move(parsed);
        break;
    }
    return true;
}
...
static bool parse_row_set(const std::string &text, int J, int L2,
    ...
    out.clear();
    std::string token;
    auto push_token = [&](const std::string &tok) -> bool {
        if (tok.empty()) {
            err = "row-set has empty entry";
            return false;
        }
        char *end = nullptr;
        long val = std::strtol(tok.c_str(), &end, 10);
        if (!end || *end != '\0') {
            err = "row-set has non-integer token: " + tok;
            return false;
        }
    ...
    for (char ch : text) {
        ...
        if (ch == ',') {
            if (!push_token(token)) return false;
            token.clear();
        } else {
            token.push_back(ch);
        }
    ...
    if (static_cast<int>(out.size()) != J) {
        err = "row-set must have " + std::to_string(J) + " entries";
        return false;
    }
    if (L2 <= 0) {
        err = "invalid L2";
    ...
    std::vector<int> seen(static_cast<size_t>(L2), 0);
    for (int v : out) {
        if (v < 0 || v >= L2) {
            err = "row-set entry out of range: " + std::to_string(v);
            return false;
        }
        if (seen[static_cast<size_t>(v)]) {
            err = "row-set has duplicates: " + std::to_string(v);
            return false;
        }
...
static int int_width(long long v) {
    int w = 1;
    if (v < 0) {
        w++;
        v = -v;
...
static void print_apm_matrix(const std::string &title, const BlockMatrix &blocks) {
    print_stdout_section(title);
    ...
    for (const auto &row : blocks) {
        for (const auto &blk : row) {
            if (!blk) continue;
            w = std::max(w, int_width(blk->a));
            w = std::max(w, int_width(blk->b));
        }
    }
    int cell_w = 2 * w + 3;  // "(a,b)"
    int row_w = std::max(2, int_width(J - 1));
    std::string empty_cell(static_cast<size_t>(cell_w), '.');
    for (int r = 0; r < J; ++r) {
        std::cout << "r" << std::setw(row_w) << r << ": ";
        for (int c = 0; c < L; ++c) {
            if (c > 0) std::cout << " ";
            const auto &blk = blocks[static_cast<size_t>(r)][static_cast<size_t>(c)];
            if (blk) {
                std::cout << "(" << std::setw(w) << blk->a << "," << std::setw(w) << blk->b << ")";
            } else {
                std::cout << empty_cell;
            }
        }
...
static void update_check_x_llr(
    const std::vector<std::vector<double>> &x_v2c,
    const std::vector<int> &sx,
    std::vector<std::vector<double>> &x_c2v,
    double eps
) {
    for (size_t c = 0; c < x_v2c.size(); ++c) {
        size_t deg = x_v2c[c].size();
        if (deg == 0) continue;
        std::vector<double> t(deg);
        for (size_t i = 0; i < deg; ++i) {
            t[i] = std::tanh(0.5 * x_v2c[c][i]);
        }
        ...
        for (size_t i = deg; i-- > 0;) {
            suffix[i] = suffix[i + 1] * t[i];
        }
        double sign = (sx[c] == 0) ? 1.0 : -1.0;
        for (size_t i = 0; i < deg; ++i) {
            double prod_excl = prefix[i] * suffix[i + 1];
            double arg = sign * prod_excl;
            x_c2v[c][i] = 2.0 * safe_atanh(arg, eps);
        }
    }
}

static void update_check_z_llr(
    const std::vector<std::vector<double>> &z_v2c,
    const std::vector<int> &sz,
    std::vector<std::vector<double>> &z_c2v,
    double eps
) {
    for (size_t c = 0; c < z_v2c.size(); ++c) {
        size_t deg = z_v2c[c].size();
        if (deg == 0) continue;
        std::vector<double> t(deg);
        for (size_t i = 0; i < deg; ++i) {
            t[i] = std::tanh(0.5 * z_v2c[c][i]);
        }
        std::vector<double> prefix(deg + 1, 1.0);
        std::vector<double> suffix(deg + 1, 1.0);
        for (size_t i = 0; i < deg; ++i) {
        ...
        double sign = (sz[c] == 0) ? 1.0 : -1.0;
        for (size_t i = 0; i < deg; ++i) {
            double prod_excl = prefix[i] * suffix[i + 1];
            double arg = sign * prod_excl;
            z_c2v[c][i] = 2.0 * safe_atanh(arg, eps);
        }
    }
}
...
static void freeze_side_llr(
    const std::vector<std::vector<int>> &checks,
    std::vector<std::vector<double>> &v2c,
    std::vector<std::vector<double>> &c2v,
    const std::vector<int> &est,
    bool use_xbit
) {
    const double Lfreeze = 20.0;
    for (int c = 0; c < static_cast<int>(checks.size()); ++c) {
        int deg = static_cast<int>(checks[c].size());
        for (int i = 0; i < deg; ++i) {
            int v = checks[c][i];
            int bit = use_xbit ? xbit(est[v]) : zbit(est[v]);
            double L = bit ? -Lfreeze : Lfreeze;
            v2c[c][i] = L;
            c2v[c][i] = L;
        }
    }
...
static JointBPResult joint_bp_decode(
    ...
    double phi00 = prior[0];
    double phi10 = prior[1];
    double phi01 = prior[2];
    double phi11 = prior[3];
    double log_phi00 = std::log(std::max(phi00, prior_eps));
    double log_phi10 = std::log(std::max(phi10, prior_eps));
    double log_phi01 = std::log(std::max(phi01, prior_eps));
    double log_phi11 = std::log(std::max(phi11, prior_eps));
    double p_x0 = phi00 + phi01;
    double p_x1 = phi10 + phi11;
    double p_z0 = phi00 + phi10;
    double p_z1 = phi01 + phi11;
    double Lg_x0 = llr_from_probs(p_x0, p_x1, prior_eps);
    double Lg_z0 = llr_from_probs(p_z0, p_z1, prior_eps);

    std::vector<std::vector<double>> x_v2c(mX), x_c2v(mX);
    for (int c = 0; c < mX; ++c) {
        int deg = static_cast<int>(x_checks[c].size());
        x_v2c[c].assign(deg, Lg_x0);
        x_c2v[c].assign(deg, 0.0);
    }

    std::vector<std::vector<double>> z_v2c(mZ), z_c2v(mZ);
    for (int c = 0; c < mZ; ++c) {
        int deg = static_cast<int>(z_checks[c].size());
        z_v2c[c].assign(deg, Lg_z0);
        z_c2v[c].assign(deg, 0.0);
    }
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        const double check_eps = 1e-12;
        if (!freeze_x_active) {
            update_check_x_llr(x_v2c, sx, x_c2v, check_eps);
        }
        if (!freeze_z_active) {
            update_check_z_llr(z_v2c, sz, z_c2v, check_eps);
        }

        for (int v = 0; v < nvars; ++v) {
            double Lx_sum = 0.0;
            for (const auto &e : var_to_x[v]) {
                Lx_sum += x_c2v[e.check][e.pos];
            }
            double Lz_sum = 0.0;
            for (const auto &e : var_to_z[v]) {
                Lz_sum += z_c2v[e.check][e.pos];
            }
            ...
            if (collect_llr) {
                double log_x0_m = logsumexp2(logI, logZ);
                double log_x1_m = logsumexp2(logX, logY);
                double log_z0_m = logsumexp2(logI, logX);
                double log_z1_m = logsumexp2(logZ, logY);
                double llr_x = log_x0_m - log_x1_m;
                double llr_z = log_z0_m - log_z1_m;
            ...
            if (!freeze_x_active) {
                for (const auto &e : var_to_x[v]) {
                    double Lx_excl = Lx_sum - x_c2v[e.check][e.pos];
                    double msg = Lg_to_x + Lx_excl;
                    if (damping > 0.0) {
                        msg = (1.0 - damping) * msg + damping * x_v2c[e.check][e.pos];
                    }
                    x_v2c[e.check][e.pos] = msg;
                }
            }
            if (!freeze_z_active) {
                for (const auto &e : var_to_z[v]) {
                    double Lz_excl = Lz_sum - z_c2v[e.check][e.pos];
                    double msg = Lg_to_z + Lz_excl;
                    if (damping > 0.0) {
                        msg = (1.0 - damping) * msg + damping * z_v2c[e.check][e.pos];
                    }
                    z_v2c[e.check][e.pos] = msg;
                }
            }
        }
        ...
        if (freeze_syn) {
            if (syn_x && !freeze_x_active) {
                ...
                freeze_side_llr(x_checks, x_v2c, x_c2v, est, true);
            }
            if (syn_z && !freeze_z_active) {
                freeze_z_active = true;
                freeze_side_llr(z_checks, z_v2c, z_c2v, est, false);
            }
        }
        int us_x_count = 0;
        int us_z_count = 0;
...
struct GeneratedETS {
    std::unordered_map<int, std::vector<ETSEntry>> by_size;
};
...
struct TannerLite {
    ...
    int n = 0;
    int m = 0;
    std::vector<std::array<int, 3>> var_neighbors;
    std::vector<std::vector<int>> check_neighbors;
};

static int common_check(
    ...
    int res = -1;
    for (int ca : a) {
        if (ca == s1 || ca == s2 || ca == s3 || ca == s4) continue;
        for (int cb : b) {
            if (ca != cb) continue;
            if (res != -1 && res != ca) return -1;
            res = ca;
        }
    }
    return res;
...
static bool build_tanner_from_checks(
    const std::vector<std::vector<int>> &checks,
    const std::vector<std::vector<EdgeRef>> &var_to_checks,
    int P,
    int L,
    ...
    if (P <= 0) return false;
    int m = static_cast<int>(checks.size());
    if (m == 0 || (m % P) != 0) return false;
    int n = static_cast<int>(var_to_checks.size());
    if (n == 0 || (n % P) != 0) return false;
    int J = m / P;
    out.P = P;
    out.J = J;
    out.L = L;
    out.n = n;
    out.m = m;
    out.check_neighbors.assign(static_cast<size_t>(m), {});
    for (int c = 0; c < m; ++c) {
        for (int v : checks[c]) {
            if (v < 0) continue;
            out.check_neighbors[c].push_back(v);
        }
    }
    out.var_neighbors.assign(static_cast<size_t>(n), std::array<int, 3>{-1, -1, -1});
    for (int v = 0; v < n; ++v) {
        const auto &edges = var_to_checks[static_cast<size_t>(v)];
        if (static_cast<int>(edges.size()) != J || J != 3) return false;
        std::array<int, 3> arr{-1, -1, -1};
        for (int i = 0; i < 3; ++i) {
            arr[i] = edges[static_cast<size_t>(i)].check;
        }
        std::sort(arr.begin(), arr.end());
        out.var_neighbors[static_cast<size_t>(v)] = arr;
    }
    return true;
}

static std::vector<ETSEntry> generate_ets_k2_tanner(const TannerLite &tg, int k) {
    std::vector<ETSEntry> entries;
    if (k < 2 || tg.J != 3) return entries;
    std::vector<int> top(static_cast<size_t>(k + 1), -1);
    std::vector<int> bottom(static_cast<size_t>(k + 1), -1);
    std::vector<int> V(static_cast<size_t>(k + 1), -1);
    std::vector<int> T(static_cast<size_t>(k), -1);
    std::vector<int> B(static_cast<size_t>(k), -1);
    std::vector<char> used(static_cast<size_t>(tg.n), 0);
    std::unordered_set<std::vector<int>, VecHash> seen;

    auto col = [&](int var) { return var / tg.P; };

    std::vector<int> deg(static_cast<size_t>(tg.m), 0);
    std::vector<int> touched;
    touched.reserve(static_cast<size_t>(6 * k + 8));

    auto check_degree_pattern = [&](int Dcheck, std::array<int, 2> &deg1_checks) -> bool {
        touched.clear();
        for (int i = 0; i <= k; ++i) {
            int vars[2] = {top[static_cast<size_t>(i)], bottom[static_cast<size_t>(i)]};
            for (int var : vars) {
                if (var < 0 || var >= tg.n) return false;
                const auto &nei = tg.var_neighbors[static_cast<size_t>(var)];
                for (int chk : nei) {
                    if (chk < 0) return false;
                    if (deg[static_cast<size_t>(chk)] == 0) {
                        touched.push_back(chk);
                    }
                    deg[static_cast<size_t>(chk)] += 1;
                    if (deg[static_cast<size_t>(chk)] > 2) {
                        for (int c : touched) deg[static_cast<size_t>(c)] = 0;
                        return false;
                    }
                }
            }
        ...
        int deg1 = 0;
        for (int chk : touched) {
            if (deg[static_cast<size_t>(chk)] == 1) {
                if (deg1 < 2) deg1_checks[static_cast<size_t>(deg1)] = chk;
                deg1++;
            }
        }
        bool ok = (deg1 == 2) && (Dcheck >= 0) &&
                  (deg[static_cast<size_t>(Dcheck)] == 2);
        for (int c : touched) deg[static_cast<size_t>(c)] = 0;
        return ok;
    };
    ...
    std::function<void(int)> dfs = [&](int i) -> void {
        if (i == k) {
            int Dcheck = common_check(tg.var_neighbors[static_cast<size_t>(top[0])],
                                      tg.var_neighbors[static_cast<size_t>(bottom[k])],
                                      V[0], V[k], T[0], B[static_cast<size_t>(k - 1)]);
            if (Dcheck < 0) return;
            std::array<int, 2> deg1_checks{-1, -1};
            if (!check_degree_pattern(Dcheck, deg1_checks)) return;
            std::vector<int> vars_vec;
            vars_vec.reserve(static_cast<size_t>(2 * k + 2));
            for (int j = 0; j <= k; ++j) {
                vars_vec.push_back(top[static_cast<size_t>(j)]);
                vars_vec.push_back(bottom[static_cast<size_t>(j)]);
            }
            std::sort(vars_vec.begin(), vars_vec.end());
            if (seen.insert(vars_vec).second) {
                ETSEntry entry;
                entry.vars = std::move(vars_vec);
                int c0 = deg1_checks[0];
                int c1 = deg1_checks[1];
                if (c0 > c1) std::swap(c0, c1);
                entry.unsat_checks = {c0, c1};
                entries.push_back(std::move(entry));
            }
            return;
        }

        const auto &n_top = tg.var_neighbors[static_cast<size_t>(top[static_cast<size_t>(i)])];
        const auto &n_bottom = tg.var_neighbors[static_cast<size_t>(bottom[static_cast<size_t>(i)])];
        for (int tchk : n_top) {
            if (tchk == V[static_cast<size_t>(i)]) continue;
            const auto &t_nei = tg.check_neighbors[static_cast<size_t>(tchk)];
            for (int top_next : t_nei) {
                if (top_next == top[static_cast<size_t>(i)]) continue;
                if (used[static_cast<size_t>(top_next)]) continue;
                int c_top_next = col(top_next);
                if (c_top_next == col(top[static_cast<size_t>(i)]) ||
                    c_top_next == col(bottom[static_cast<size_t>(i)])) continue;

                for (int bchk : n_bottom) {
                    if (bchk == V[static_cast<size_t>(i)] || bchk == tchk) continue;
                    const auto &b_nei = tg.check_neighbors[static_cast<size_t>(bchk)];
                    for (int bottom_next : b_nei) {
                        if (bottom_next == bottom[static_cast<size_t>(i)]) continue;
                        if (used[static_cast<size_t>(bottom_next)]) continue;
                        int c_bottom_next = col(bottom_next);
                        if (c_bottom_next == col(top[static_cast<size_t>(i)]) ||
                            c_bottom_next == col(bottom[static_cast<size_t>(i)])) continue;
                        if (c_bottom_next == c_top_next) continue;

                        int v_next = common_check(tg.var_neighbors[static_cast<size_t>(top_next)],
                                                  tg.var_neighbors[static_cast<size_t>(bottom_next)],
                                                  tchk, bchk, V[static_cast<size_t>(i)]);
                        if (v_next < 0) continue;
                        if (v_next == tchk || v_next == bchk ||
                            v_next == V[static_cast<size_t>(i)]) continue;

                        top[static_cast<size_t>(i + 1)] = top_next;
                        bottom[static_cast<size_t>(i + 1)] = bottom_next;
                        V[static_cast<size_t>(i + 1)] = v_next;
                        T[static_cast<size_t>(i)] = tchk;
                        B[static_cast<size_t>(i)] = bchk;
                        used[static_cast<size_t>(top_next)] = 1;
                        used[static_cast<size_t>(bottom_next)] = 1;
                        dfs(i + 1);
                        used[static_cast<size_t>(top_next)] = 0;
                        used[static_cast<size_t>(bottom_next)] = 0;
                    }
                }
            }
        }
    ...
    for (int v0 = 0; v0 < tg.m; ++v0) {
        const auto &nei = tg.check_neighbors[static_cast<size_t>(v0)];
        int deg = static_cast<int>(nei.size());
        for (int i = 0; i < deg; ++i) {
            for (int j = 0; j < deg; ++j) {
                if (i == j) continue;
                int t0 = nei[static_cast<size_t>(i)];
                int b0 = nei[static_cast<size_t>(j)];
                top[0] = t0;
                bottom[0] = b0;
                V[0] = v0;
                used[static_cast<size_t>(t0)] = 1;
                used[static_cast<size_t>(b0)] = 1;
                dfs(0);
                used[static_cast<size_t>(t0)] = 0;
                used[static_cast<size_t>(b0)] = 0;
            }
        }
    }
    return entries;
...
static GeneratedETS generate_ets_entries_k2_tanner(
    const std::vector<std::vector<int>> &checks,
    const std::vector<std::vector<EdgeRef>> &var_to_checks,
    int P,
    int max_k
    ...
    GeneratedETS out;
    if (max_k < 2) return out;
    int nvars = static_cast<int>(var_to_checks.size());
    if (nvars <= 0 || P <= 0 || (nvars % P) != 0) return out;
    int L = nvars / P;
    TannerLite tg;
    if (!build_tanner_from_checks(checks, var_to_checks, P, L, tg)) return out;
    for (int k = 2; k <= max_k; ++k) {
        int target_vars = 2 * (k + 1);
        auto entries = generate_ets_k2_tanner(tg, k);
        if (!entries.empty() || target_vars > 0) {
            out.by_size[target_vars] = std::move(entries);
        }
    }
    return out;
}

static bool write_ets_file(const std::string &path,
                           const std::vector<ETSEntry> &x_entries,
                           const std::vector<ETSEntry> &z_entries,
                           int P,
                           bool write_x,
                           bool write_z) {
    ...
    if (!out) return false;
    auto emit_entries = [&](const std::vector<ETSEntry> &entries, char side) {
        int idx = 0;
        for (const auto &entry : entries) {
            out << side << " " << idx++ << " vars=";
            for (size_t i = 0; i < entry.vars.size(); ++i) {
                int col = 0, vidx = 0;
                decode_var(entry.vars[i], P, col, vidx);
                if (i) out << ",";
                out << "V" << col << ":" << vidx;
            }
            out << " unsat=";
            for (size_t i = 0; i < entry.unsat_checks.size(); ++i) {
                int row = 0, cidx = 0;
                decode_check(entry.unsat_checks[i], P, row, cidx);
                if (i) out << ",";
                out << "C" << row << ":" << cidx;
            }
            out << "\n";
    ...
    if (write_x) emit_entries(x_entries, 'X');
    if (write_z) emit_entries(z_entries, 'Z');
    return true;
}

static int ets_check_distance(const ETSEntry &entry,
                              const std::vector<std::vector<EdgeRef>> &var_to_checks) {
    if (entry.unsat_checks.size() < 2) return -1;
    std::unordered_map<int, int> check_map;
    check_map.reserve(entry.vars.size() * 3 + 4);
    std::vector<int> check_ids;
    for (int v : entry.vars) {
        if (v < 0 || v >= static_cast<int>(var_to_checks.size())) continue;
        for (const auto &edge : var_to_checks[v]) {
            int chk = edge.check;
            if (check_map.find(chk) == check_map.end()) {
                int idx = static_cast<int>(check_ids.size());
                check_ids.push_back(chk);
                check_map[chk] = idx;
            }
        }
    }
    auto it0 = check_map.find(entry.unsat_checks[0]);
    auto it1 = check_map.find(entry.unsat_checks[1]);
    if (it0 == check_map.end() || it1 == check_map.end()) return -1;
    int num_checks = static_cast<int>(check_ids.size());
    int num_vars = static_cast<int>(entry.vars.size());
    int total_nodes = num_checks + num_vars;
    std::vector<std::vector<int>> adj(total_nodes);
    for (int i = 0; i < num_vars; ++i) {
        int v = entry.vars[static_cast<size_t>(i)];
        if (v < 0 || v >= static_cast<int>(var_to_checks.size())) continue;
        int vnode = num_checks + i;
        for (const auto &edge : var_to_checks[v]) {
            auto it = check_map.find(edge.check);
            if (it == check_map.end()) continue;
            int cnode = it->second;
            adj[vnode].push_back(cnode);
            adj[cnode].push_back(vnode);
        }
    }
    std::vector<int> dist(total_nodes, -1);
    std::deque<int> dq;
    int start = it0->second;
    int target = it1->second;
    dist[start] = 0;
    dq.push_back(start);
    while (!dq.empty()) {
        int cur = dq.front();
...
static GeneratedETS generate_ets_entries(
    const std::vector<std::vector<int>> &checks,
    const std::vector<std::vector<EdgeRef>> &var_to_checks,
    int P,
    int max_k
    ...
    GeneratedETS out;
    if (P <= 0 || max_k <= 0) return out;
    int nvars = static_cast<int>(var_to_checks.size());
    if (nvars <= 0) return out;
    int total_checks = static_cast<int>(checks.size());
    if (total_checks % P != 0) return out;
    int J = total_checks / P;
    if (J == 3) {
        return generate_ets_entries_k2_tanner(checks, var_to_checks, P, max_k);
    }
    int max_vars = 2 * (max_k + 1);
    if (max_vars < 6) return out;

    std::vector<int> var_checks_flat(static_cast<size_t>(nvars) * J, -1);
    std::vector<char> var_valid(nvars, 1);
    for (int v = 0; v < nvars; ++v) {
        int filled = 0;
        for (const auto &edge : var_to_checks[v]) {
            int row = edge.check / P;
            if (row < 0 || row >= J) continue;
            var_checks_flat[static_cast<size_t>(v) * J + row] = edge.check;
            filled++;
        }
        if (filled != J) var_valid[v] = 0;
    }

    auto run_search = [&](int target_vars) {
        std::unordered_set<std::string> seen;
        std::vector<ETSEntry> entries;
        std::vector<int> vars;
        vars.reserve(static_cast<size_t>(target_vars));
        std::vector<uint8_t> check_deg(static_cast<size_t>(total_checks), 0);
        std::vector<int> odd_pos(static_cast<size_t>(total_checks), -1);
        std::vector<int> odd_checks;
        odd_checks.reserve(static_cast<size_t>(3 * target_vars));
        std::vector<int> active_pos(static_cast<size_t>(total_checks), -1);
        std::vector<int> active_checks;
        active_checks.reserve(static_cast<size_t>(3 * target_vars));
        std::vector<int> odd_per_row(static_cast<size_t>(J), 0);
        std::vector<int> active_per_row(static_cast<size_t>(J), 0);
        std::vector<char> var_in_set(nvars, 0);
        std::vector<char> candidate_mark(nvars, 0);
        std::vector<int> candidates;
        candidates.reserve(512);

        auto mark_active = [&](int chk) {
            if (active_pos[chk] >= 0) return;
            active_pos[chk] = static_cast<int>(active_checks.size());
            active_checks.push_back(chk);
            int row = chk / P;
            if (row >= 0 && row < J) active_per_row[static_cast<size_t>(row)]++;
        };
        auto unmark_active = [&](int chk) {
            int pos = active_pos[chk];
            if (pos < 0) return;
            int last = active_checks.back();
            active_checks[static_cast<size_t>(pos)] = last;
            active_pos[last] = pos;
            active_checks.pop_back();
            active_pos[chk] = -1;
            int row = chk / P;
            if (row >= 0 && row < J) active_per_row[static_cast<size_t>(row)]--;
        };
        auto mark_odd = [&](int chk) {
            if (odd_pos[chk] >= 0) return;
            odd_pos[chk] = static_cast<int>(odd_checks.size());
            odd_checks.push_back(chk);
            int row = chk / P;
            if (row >= 0 && row < J) odd_per_row[static_cast<size_t>(row)]++;
        };
        auto unmark_odd = [&](int chk) {
            int pos = odd_pos[chk];
            if (pos < 0) return;
            int last = odd_checks.back();
            odd_checks[static_cast<size_t>(pos)] = last;
            odd_pos[last] = pos;
            odd_checks.pop_back();
            odd_pos[chk] = -1;
            int row = chk / P;
        ...
        auto add_var = [&](int v) -> bool {
            if (var_in_set[v]) return false;
            if (!var_valid[v]) return false;
            for (int r = 0; r < J; ++r) {
                int chk = var_checks_flat[static_cast<size_t>(v) * J + r];
                if (chk < 0) return false;
                if (check_deg[chk] >= 2) return false;
            }
            var_in_set[v] = 1;
            for (int r = 0; r < J; ++r) {
                int chk = var_checks_flat[static_cast<size_t>(v) * J + r];
                uint8_t d = check_deg[chk];
                if (d == 0) {
                    check_deg[chk] = 1;
                    mark_active(chk);
                    mark_odd(chk);
        ...
        auto remove_var = [&](int v) {
            if (!var_in_set[v]) return;
            for (int r = 0; r < J; ++r) {
                int chk = var_checks_flat[static_cast<size_t>(v) * J + r];
                uint8_t d = check_deg[chk];
                if (d == 2) {
        ...
        std::function<void(int, int)> dfs = [&](int root, int target_row) {
            int size = static_cast<int>(vars.size());
            if (size > target_vars) return;
            if (size == target_vars) {
                if (odd_checks.size() == 2) {
                    int row0 = odd_checks[0] / P;
                    int row1 = odd_checks[1] / P;
                    if (row0 == row1 && row0 == target_row) {
                        std::vector<int> sorted_vars = vars;
                        std::sort(sorted_vars.begin(), sorted_vars.end());
                        std::string key;
                        key.resize(sorted_vars.size() * sizeof(int));
                        std::memcpy(key.data(), sorted_vars.data(), key.size());
                        if (seen.insert(key).second) {
                            ETSEntry entry;
                            entry.vars = std::move(sorted_vars);
                            int c0 = odd_checks[0];
                            int c1 = odd_checks[1];
                            if (c0 > c1) std::swap(c0, c1);
                            entry.unsat_checks = {c0, c1};
            ...
            int remaining = target_vars - size;
            for (int r = 0; r < J; ++r) {
                int limit = (r == target_row) ? (remaining + 2) : remaining;
                if (odd_per_row[static_cast<size_t>(r)] > limit) return;
            }
            int k_limit = target_vars / 2;
            for (int r = 0; r < J; ++r) {
                int max_active = (r == target_row) ? (k_limit + 1) : k_limit;
                if (active_per_row[static_cast<size_t>(r)] > max_active) return;
            }

            int pivot_check = -1;
            for (int chk : odd_checks) {
                int row = chk / P;
                if (row != target_row || odd_per_row[static_cast<size_t>(row)] > 2) {
                    pivot_check = chk;
                    break;
                }
            }
            ...
            auto collect_candidates = [&](int chk) {
                if (chk < 0 || chk >= total_checks) return;
                for (int v : checks[chk]) {
                    if (v < root || v >= nvars) continue;
                    if (var_in_set[v]) continue;
                    if (!var_valid[v]) continue;
                    if (candidate_mark[v]) continue;
                    bool ok = true;
                    for (int r = 0; r < J; ++r) {
                        int c = var_checks_flat[static_cast<size_t>(v) * J + r];
                        if (c < 0 || check_deg[c] >= 2) {
                            ok = false;
                            break;
                        }
                    }
            ...
            if (pivot_check >= 0) {
                collect_candidates(pivot_check);
            } else {
                for (int chk : active_checks) {
                    collect_candidates(chk);
                }
            }
            std::sort(candidates.begin(), candidates.end());
            for (int v : candidates) {
                if (!add_var(v)) continue;
                vars.push_back(v);
                dfs(root, target_row);
                vars.pop_back();
                remove_var(v);
        ...
        for (int target_row = 0; target_row < J; ++target_row) {
            for (int root = 0; root < nvars; ++root) {
                if (!var_valid[root]) continue;
                if (!add_var(root)) continue;
                vars.push_back(root);
                dfs(root, target_row);
                vars.pop_back();
                remove_var(root);
    ...
    for (int k = 2; k <= max_k; ++k) {
        int target_vars = 2 * (k + 1);
        auto entries = run_search(target_vars);
        if (!entries.empty() || target_vars <= max_vars) {
            out.by_size[target_vars] = std::move(entries);
        }
    }
...
static void print_usage(const char *prog) {
    ...
    print_help_section("Active Rows");
    std::cerr << "  Params file can include # active_set=0,2,4 (or # active_row=...).\n";
    std::cerr << "  When present, those indices are used as active rows.\n";
...
int main(int argc, char **argv) {
    ...
    auto now = std::chrono::system_clock::now();
        ...
        time_oss << std::put_time(&tm_buf, "%Y%m%d_%H%M%S");
    ...
    std::string log_filename = logs_dir + "/result_P" + std::to_string(params.P) + "_" + params_path + "_" + time_str + ".log";
    std::ofstream log_file(log_filename);
    
    TeeBuf tee(std::cout.rdbuf());
    std::streambuf* orig_cout_buf = std::cout.rdbuf();

    if (log_file.is_open()) {
        std::cout.rdbuf(&tee); // 出力の分岐(蓄積)を開始
        std::cout << "=== 実行コマンド ===\n" << cmd_str << "\n====================\n\n";
    }
    ...
    if (!params.active_set.empty()) {
        row_set = params.active_set;
    } else {
        row_set.reserve(static_cast<size_t>(params.J));
        for (int r = 0; r < params.J; ++r) row_set.push_back(r);
    }
    auto hx_blocks = build_block_matrix_hx(params.P, params.J, params.L2, params.a, params.b, params.c, params.d, row_set);
    auto hz_blocks = build_block_matrix_hz(params.P, params.J, params.L2, params.a, params.b, params.c, params.d, row_set);
    ...
    if (!orth.orthogonal) {
        std::cerr << "hx_hz_orthogonal=false odd_pairs=" << orth.odd_pairs;
        if (orth.first_x >= 0 && orth.first_z >= 0) {
            std::cerr << " first_odd_pair=" << orth.first_x << "," << orth.first_z
                      << " overlap=" << orth.first_overlap;
        }
        std::cerr << "\n";
        return 1;
    }
    ...
    print_apm_matrix("HX APM", hx_blocks);
    print_apm_matrix("HZ APM", hz_blocks);

    const int ets_max_k = 5;
    if (enable_pp) {
        ...
        std::string ets8_path = log_dir + "/ets_8_2.txt";
        std::string ets10_path = log_dir + "/ets_10_2.txt";
        bool ets6_x_missing = !file_exists(ets6_x_path);
        bool ets6_z_missing = !file_exists(ets6_z_path);
        bool ets8_missing = !file_exists(ets8_path);
        bool ets8p4_missing = !file_exists(ets8_path4_path);
        bool ets10_missing = !file_exists(ets10_path);
        bool ets12_x_missing = !file_exists(ets12_x_path);
        bool ets12_z_missing = !file_exists(ets12_z_path);
        if (ets6_x_missing || ets6_z_missing || ets8_missing || ets8p4_missing ||
            ets10_missing || ets12_x_missing || ets12_z_missing) {
            GeneratedETS gen_x = generate_ets_entries(
                x_checks, var_to_x, static_cast<int>(params.P), ets_max_k
            );
            GeneratedETS gen_z = generate_ets_entries(
                z_checks, var_to_z, static_cast<int>(params.P), ets_max_k
            );
            auto sort_entries = [](std::vector<ETSEntry> &entries) {
                std::sort(entries.begin(), entries.end(),
                          [](const ETSEntry &a, const ETSEntry &b) {
                              return a.vars < b.vars;
                          });
            };
            auto &x6 = gen_x.by_size[6];
            auto &z6 = gen_z.by_size[6];
            auto &x8 = gen_x.by_size[8];
            auto &z8 = gen_z.by_size[8];
            auto &x10 = gen_x.by_size[10];
            auto &z10 = gen_z.by_size[10];
            auto &x12 = gen_x.by_size[12];
            auto &z12 = gen_z.by_size[12];
            sort_entries(x6);
            sort_entries(z6);
            sort_entries(x8);
            sort_entries(z8);
            ...
            std::vector<ETSEntry> x8_path4;
            std::vector<ETSEntry> x8_normal;
            std::vector<ETSEntry> z8_path4;
            std::vector<ETSEntry> z8_normal;
            for (const auto &entry : x8) {
                int dist = ets_check_distance(entry, var_to_x);
                if (dist == 4) {
                    x8_path4.push_back(entry);
                } else {
                    x8_normal.push_back(entry);
                }
            }
            for (const auto &entry : z8) {
                int dist = ets_check_distance(entry, var_to_z);
                if (dist == 4) {
                    z8_path4.push_back(entry);
                } else {
                    z8_normal.push_back(entry);
                }
            }
            sort_entries(x8_path4);
            sort_entries(z8_path4);
            sort_entries(x8_normal);
            sort_entries(z8_normal);

            if (ets6_x_missing || ets6_z_missing) {
                if (ets6_x_path == ets6_z_path) {
                    if (ets6_x_missing) {
                        write_ets_file(ets6_x_path, x6, z6, static_cast<int>(params.P), true, true);
                    }
                } else {
                    if (ets6_x_missing) {
                        write_ets_file(ets6_x_path, x6, {}, static_cast<int>(params.P), true, false);
                    }
                    if (ets6_z_missing) {
                        write_ets_file(ets6_z_path, {}, z6, static_cast<int>(params.P), false, true);
                    }
                }
            }
            if (ets8_missing) {
                write_ets_file(ets8_path, x8_normal, z8_normal, static_cast<int>(params.P), true, true);
            }
            if (ets8p4_missing) {
                write_ets_file(ets8_path4_path, x8_path4, z8_path4, static_cast<int>(params.P), true, true);
            }
            if (ets10_missing) {
                write_ets_file(ets10_path, x10, z10, static_cast<int>(params.P), true, true);
            }
            if (ets12_x_missing || ets12_z_missing) {
                if (ets12_x_path == ets12_z_path) {
                    if (ets12_x_missing) {
                        write_ets_file(ets12_x_path, x12, z12, static_cast<int>(params.P), true, true);
                    }
                } else {
                    if (ets12_x_missing) {
                        write_ets_file(ets12_x_path, x12, {}, static_cast<int>(params.P), true, false);
                    }
                    if (ets12_z_missing) {
                        write_ets_file(ets12_z_path, {}, z12, static_cast<int>(params.P), false, true);
                    }
                }
            }
        }
...
if (log_file.is_open()) {
        std::cout.rdbuf(orig_cout_buf); // バッファを元に戻す
        tee.write_to_file(log_file);    // 上書き反映済みの綺麗なログをファイルに一括書き出し
        log_file.close();
    }
...
/*
./jointbp_llr --simulate --p 0.04000000 --trials 1000 --max-iter 1000   --flip-hist 5 --damping 0.000000 --report-every 20 --no-pp --params ./params/P_768_act_024_nc_0-1_0-3_20260410_213022.txt
*/