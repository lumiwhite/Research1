...
void init_state(int step, const int *a_vec, const int *b_vec, SearchState *state, unsigned int *seed, const int *valid_a_list, int num_valid_a) {
    ...
    for (int a_i = 0; a_i < num_valid_a; a_i++) {
        ...
        for(int s=0; s<step; s++) {
            ...
            // 【変更点】アクティブインデックス集合の分散配置 (S' = {0, 2, 4}) と段階的アプローチの適用
            // r = 0, 2, 4 は直交性維持のため可換 (commute_required = true) を強制する。
            // r = 1, 3, 5 は非可換にすることが許容される。
            // 探索ステップ1として、最小限の制約である「(i=0, j=3)の1組のみ非可換」を要求する。
            int r = (i + j) % L_H;
...
int main(int argc, char *argv[]) {
    ...
    // printf("※アクティブインデックス分散配置 (S'={0,2,4}) 用の制約緩和モード\n");
    fflush(stdout);
...
// a_vec: [253, 337, 505, 1, 225, 225, 687, 295, 197, 99, 295, 393]
// b_vec: [658, 296, 68, 168, 592, 256, 469, 329, 406, 21, 301, 140]