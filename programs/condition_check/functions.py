...
def gen_c_constraints(a_vec, cycles, h_x, h_z):
    def inv(val):
        try:
            return pow(val, -1, P)
        except ValueError:
...
def gen_constraints(Gb, a_vec, cycles, h_x, h_z):
    ...
    for row in Gb:
        # NumPy配列をリストに変換し、Pでの剰余を取って正の数に統一する
        row_mod = [(int(x) % P + P) % P for x in row]
        row_tuple = tuple(row_mod)
        if row_tuple not in seen_vectors:
            unique_forbidden_vectors.append(row_mod) # 列ベクトルとして保存
