...
def get_cycles():
    """ 完全2部グラフ K_3,12 から長さ4および6のサイクルをすべて抽出する（計1518個） """
    raw_cycles = []
    
    # 長さ4のサイクル抽出 (検査ノード2つ、変数ノード2つの組み合わせ: 198個)
    for c0, c1 in itertools.combinations(C, 2):
        for v0, v1 in itertools.combinations(V, 2):
            raw_cycles.append([c0, v0, c1, v1])
            
    # 長さ6のサイクル抽出 (検査ノード3つ、変数ノード3つの組み合わせ: 1320個)
...
def write_cycles_header(cx, cz, filename="cycles_data012.h"):
    """ C言語用のヘッダーファイルとしてエクスポートする """
    with open(filename, "w") as f:
        f.write("#ifndef CYCLES_DATA_H\n#define CYCLES_DATA_H\n\n")
...
if __name__ == "__main__":
    ...
    # 抽出されたサイクルの確認出力
    print(f"\n総サイクル数: {len(raw_cycles)}")
    
    # 2. 関数インデックス列への変換
    cx, cz = gen_function_sequences(raw_cycles)

    # 3. C言語用ヘッダーファイルへの書き出し
    write_cycles_header(cx, cz)
    print("cycles_data012.h のエクスポートが完了した。")